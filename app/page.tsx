...
interface SummaryResponse {
  ...
  targets: Target[];
  orders: Pick<Order, "jan_code" | "department" | "person" | "customer_name" | "product_name" | "slip_date">[];
}
...
export default function DashboardPage() {
  ...
  // 商材サマリー計算
  const productSummaries: ProductSummary[] = (data?.products ?? []).map((product) => {
    ...
    const target =
      department === COMPANY_DEPT
        ? (data?.targets ?? [])
            .filter((t) => t.product_id === product.id && t.department_id === null)
  ...
  const genreGroups = Array.from(
    new Map(
      (data?.products ?? []).map((p) => [p.genre?.name ?? "その他", p.genre])
    ).entries()
  );
